...
for i in range(3):
    for trial in range(trial_number):
        sw = swtaskreactivation.SlowwaveReactivationDecrease(epoch_length=epolen, epoch_number=eponum, pre_w_initial=pre_w_initial, reac_state=state_list[i], reac_neuron=reactivation_neuron, reac_rate=0.0075, wake_epoch_number=epowake, wake_rate=0.005, w_pre_list_initial=pre_w_initial*np.ones(100), plasticity_option=True)
        sw.simulate()
...
np.save('data/result_weight_globalpre2', result_weight_globalpre)
np.save('data/result_weight_localpre2', result_weight_localpre)
np.save('data/result_reactivation_global2', result_reactivation_global)
np.save('data/result_reactivation_local2', result_reactivation_local)
np.save('data/result_perf2', result_perf)
np.save('data/result_perf_beforesleep2', result_perf_beforesleep)
np.save('data/base_performance2', base_performance)
