...
def calculate_stdp_slowwave(delta_t, stim_state):
    ...
    success = False
    while not success:
        cal_stdp = slowwavemodel.SlowwaveSTDP(epoch_length=50000, pre_w_initial=0.5, delta_t=delta_t, stim_state=stim_state, stim_neuron=20)
        cal_stdp.simulate()
...
def calculate_stdp_preonly_slowwave(stim_state):
    ...
    success = False
    while not success:
        cal_stdp = slowwavemodel.SlowwaveSTDPPreonly(epoch_length=50000, pre_w_initial=0.5, stim_state=stim_state, stim_neuron=20)
        cal_stdp.simulate()
        if cal_stdp.stim_count == cal_stdp.stim_num_total:
            success = True
    return (np.mean(cal_stdp.w_pre[0:cal_stdp.stim_neuron]) - cal_stdp.pre_w_initial) / cal_stdp.pre_w_initial * 100
...
def calculate_and_plot_stdp(file_name):
    ...
    #plot STDP curve
    for i in range(4):
        figplot.plot_stdp_curve(delta_t=delta_t_list, g=result_stdp[i,:], preonly=result_stdp_pre[i], g_min=-3.5, g_max=12.0, t_min=np.min(delta_t_list), t_max=np.max(delta_t_list), aspect=0.75, fig=fig, plot_x=3, plot_y=4, plot_area=i+1)
        figplot.plot_stdp_curve(delta_t=delta_t_list, g=result_stdp[i,:], preonly=result_stdp_pre[i], g_min=-3.5, g_max=1.5, t_min=np.min(delta_t_list), t_max=np.max(delta_t_list), aspect=0.75, fig=fig, plot_x=3, plot_y=4, plot_area=i+5)
        figplot.plot_stdp_curve(delta_t=delta_t_list, g=result_stdp[i,:], preonly=result_stdp_pre[i], g_min=-12.0, g_max=16.0, t_min=np.min(delta_t_list), t_max=np.max(delta_t_list), aspect=0.75, fig=fig, plot_x=3, plot_y=4, plot_area=i+9)
    plt.savefig(file_name)

calculate_and_plot_stdp('figure/slowwave/stdp.pdf')
