...
def plot_u_distribution(file_name):
    ...
    #plt.savefig(file_name)
    return np.ravel(u_distribution)

plot_state_fr('figure/slowwave/firingrate.pdf')
u_dist = plot_u_distribution('figure/slowwave/udistribution.pdf')

u_hist = np.histogram(u_dist, bins=160)
def utopbottom(index):
    u_ans = (u_hist[1][index] + u_hist[1][index+1])/2
    print(index)
    print(u_ans)
utopbottom(np.argmax(u_hist[0]))
utopbottom(np.argmax(u_hist[0][50:160])+50)
utopbottom(np.argmin(u_hist[0][25:80])+25)
